theory ExhaleRelML
imports Boogie_Lang.HelperML ExprWfRelML TotalViperSimulation.ExhaleRel ViperBoogieHelperML CPGHelperML TotalViperSimulation.PredicateRel ExhalePredAccRel
begin
...
ML \<open>
  ...
  fun upd_exhale_field_acc_tac ctxt (info: basic_stmt_rel_info) exp_rel_info =
    ...
    (Rmsg' "UpdExhField Wf TyRepr" (resolve_tac ctxt [#wf_ty_repr_thm info]) ctxt) THEN'
    ...
    (Rmsg' "UpdExhField MaskUpdateWf" (resolve_tac ctxt [@{thm mask_update_wf_concrete} OF [#ctxt_wf_thm info, #wf_ty_repr_thm info]] THEN'
                                       assm_full_simp_solved_tac ctxt) ctxt) THEN'
    (Rmsg' "UpdExhField MaskReadWf" (resolve_tac ctxt [@{thm mask_read_wf_concrete} OF [#ctxt_wf_thm info, #wf_ty_repr_thm info]] THEN'
                                     assm_full_simp_solved_tac ctxt) ctxt) THEN'
    (Rmsg' "UpdExhField MaskUpdateBpl" (assm_full_simp_solved_with_thms_tac [@{thm update_mask_concrete_def}, #ty_repr_def_thm info] ctxt) ctxt) THEN'
    (Rmsg' "UpdExhField MaskReadBpl" (assm_full_simp_solved_with_thms_tac [@{thm read_mask_concrete_def}, #ty_repr_def_thm info] ctxt) ctxt) THEN'
  ...
  fun store_temporary_perm_pred_exh_tac ctxt (info: basic_stmt_rel_info) exp_rel_info lookup_aux_var_ty_thm =
    store_temporary_perm_tac
         ...
         exp_rel_info
         lookup_aux_var_ty_thm
         (fn ctxt => (resolve_tac ctxt @{thms exhale_pred_acc_rel_assms_perm_eval'}) THEN' fastforce_tac ctxt [])
  ...
  fun upd_exhale_pred_acc_tac ctxt (info: basic_stmt_rel_info) pred_name exp_rel_info =
    ...
    (Rmsg' "exh pred upd MaskVarDefDiff" (assm_full_simp_solved_with_thms_tac [#tr_def_thm info] ctxt) ctxt) THEN'
