theory StmtRelML
imports Boogie_Lang.HelperML ExprWfRelML TotalViperSimulation.StmtRel InhaleRelML ExhaleRelML PredicateRelML ViperBoogieHelperML
begin
...
ML \<open>
  ...
  fun prove_ctxt_pred_self_framing_sat_tac ctxt =
 ...
 fun stmt_rel_tac ctxt (info: ('a, 'i, 'e) stmt_rel_info) (stmt_rel_hint: 'a stmt_rel_hint) =
   ...
   (* scopes are handled here, because in the Scala AST they are bound to sequential composition
    ...
    | ScopeHint (lookup_decl_thm, body_hint) =>
       ...
       (Rmsg' "Scope ctxt pred self framing" (prove_ctxt_pred_self_framing_sat_tac ctxt) ctxt) THEN'
...
ML \<open>
  ...
  fun field_assign_rel_tac ctxt (info : basic_stmt_rel_info) atomic_hint =
    (case atomic_hint of
       ...
       (Rmsg' "FieldAssign CtxtPredSelfFraming" (prove_ctxt_pred_self_framing_sat_tac ctxt) ctxt) THEN'
  ...
  fun atomic_rel_inst_tac ctxt (inhale_info: atomic_inhale_rel_hint inhale_rel_info) (exhale_info: atomic_exhale_rel_hint exhale_rel_info) (basic_info : basic_stmt_rel_info) (atomic_hint : atomic_rel_hint)  =
    (case atomic_hint of
     ...
     | ExhaleHint (NormalExhCompleteHint exh_complete_hint) =>
        ...
        (Rmsg' "AtomicExh1 CtxtPredSelfFraming" (prove_ctxt_pred_self_framing_sat_tac ctxt) ctxt) THEN'
     ...
     | MethodCallHint (callee_name, rets_lookup_decl_thms, inhale_info_call, exhale_info_call, exh_pre_complete_hint, inh_post_complete_hint) =>
        ...
        (Rmsg' "MethodCall CtxtPredSelfFraming" (prove_ctxt_pred_self_framing_sat_tac ctxt) ctxt) THEN'
