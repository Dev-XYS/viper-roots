...
inductive fold_rel :: "'a total_context \<Rightarrow> predicate_ident \<Rightarrow> ('a val list) \<Rightarrow> preal \<Rightarrow> 'a full_total_state \<Rightarrow> 'a result_total \<Rightarrow> bool" where
  ...
  "\<lbrakk> ViperLang.predicates (program_total ctxt) pid = Some pdecl;
     ...
     \<omega>0 = \<omega>\<lparr> get_store_total := nth_option vs \<rparr>;
     red_exhale ctxt \<omega>0 (syntactic_mult (Rep_preal q) pbody) \<omega>0 (RNormal \<omega>1');
     ...
     q > 0;
     \<omega>' = add_to_lpm_nonzero_total_full \<omega>1 (pid,vs) (Abs_posreal q) nm_exh
   \<rbrakk> \<Longrightarrow>
...
| FoldRelFailure:
  "\<lbrakk> ViperLang.predicates (program_total ctxt) pid = Some pdecl;
     ...
     vals_well_typed (absval_interp_total ctxt) vs (predicate_decl.args pdecl);
     \<omega>0 = \<omega>\<lparr> get_store_total := nth_option vs \<rparr>;
     red_exhale ctxt \<omega>0 (syntactic_mult (Rep_preal q) pbody) \<omega>0 RFailure
   \<rbrakk> \<Longrightarrow>
   fold_rel ctxt pid vs q \<omega> RFailure"
