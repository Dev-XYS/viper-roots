...
definition mono_prop_downward_sub_mask_total :: "('a total_state \<Rightarrow> bool) \<Rightarrow> bool" where
  "mono_prop_downward_sub_mask_total P \<equiv> \<forall>\<phi> lp p nm.
     P \<phi> \<longrightarrow>
     Some (p,nm) = get_fnm_total \<phi> lp \<longrightarrow>
     P (\<phi>\<lparr> get_nm_total := nm \<rparr>)"
...
lemma exhale_0_state_same:
  assumes "red_exhale ctxt \<omega>0 (syntactic_mult 0 A) \<omega> (RNormal \<omega>')"
  shows "\<omega>' = \<omega>"
  using assms
...
proof -
  obtain pred_decl pred_body \<omega>0 \<omega>1 \<omega>1' nm_exh where
    ...
    \<omega>0: "\<omega>0 = \<omega>\<lparr> get_store_total := nth_option vs \<rparr>" and
