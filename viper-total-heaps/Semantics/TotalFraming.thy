...
text \<open>\<^term>\<open>p\<close> must range only over strictly positive scaling factors: a negative \<open>p\<close> makes
  \<^const>\<open>red_inhale\<close> fail on any accessibility predicate, and \<open>p = 0\<close> can turn a self-framing
  assertion into a failing one, e.g. \<open>acc(x.f) \<and>\<and> x.f > 0\<close> is self-framing at \<open>p = 1\<close> but not at
