...
(* general information for tactics *)
  ...
  type basic_stmt_rel_info = {
      ...
      predicate_data_table: predicate_data Symtab.table,
...
fun upd_def_var_tac_aux (basic_stmt_rel_info : basic_stmt_rel_info) lookup_ty_new_var_thm errorMsgPrefix ctxt =
  ...
  (Rmsg' (errorMsgPrefix^"DefVar4") (assm_full_simp_solved_with_thms_tac [lookup_ty_new_var_thm, #ty_repr_def_thm basic_stmt_rel_info] ctxt) ctxt) THEN'
...
\<comment> \<open>The following lemma solves schematic goals that look like \<open>Some (TConSingle ''Ref'')] = map Some (?tys_bpl vs p)\<close>.
    Todo: Move somewhere else.\<close>

lemma map_Some_inv:
  assumes "list_all (HOL.Not \<circ> Option.is_none) l"
  shows "l = map Some (map the l)"
  using assms
  by (induct l; simp add: Option.is_none_def)
...
fun intro_fact_mask_lookup_reduction ctxt (info: basic_stmt_rel_info) exp_rel_info exp_rel_perm_access_thm vpr_rcv_red_tac =
  ...
  (Rmsg' "intro mask lookup MaskReadWf" (resolve_tac ctxt @{thms mask_read_wf_concrete} THEN'
  ...
  (Rmsg' "intro mask lookup red 9" (assm_full_simp_solved_with_thms_tac [#ty_repr_def_thm info] ctxt) ctxt) THEN'
...
fun prove_ploc_reduce ctxt (info: basic_stmt_rel_info) pred_name exp_rel_info =
  ...
  (Rmsg' "intro mask lookup red 5" (blast_tac ctxt) ctxt) THEN'
  (Rmsg' "intro mask lookup 1" (simp_only_tac @{thms exhale_pred_acc_rel_assms_def} ctxt) ctxt) THEN'
  (Rmsg' "intro mask lookup 2" (fastforce_tac ctxt []) ctxt) THEN'
  (Rmsg' "intro mask lookup 1" (simp_only_tac @{thms exhale_pred_acc_rel_assms_def} ctxt) ctxt) THEN'
  (Rmsg' "intro mask lookup 2" (fastforce_tac ctxt []) ctxt) THEN'
  (Rmsg' "intro mask lookup 5" (simp_tac_with_thms [] ctxt) ctxt) THEN'
  (Rmsg' "intro mask lookup 6" (simp_tac_with_thms [#vpr_program_ctxt_eq_thm info, #predicate_lookup_thm (lookup_predicate_data info pred_name)] ctxt) ctxt) THEN'
  (Rmsg' "intro mask lookup 8" (simp_tac_with_thms @{thms predicate_decl.defs} ctxt) ctxt) THEN'
  (Rmsg' "intro mask lookup 5" (simp_tac_with_thms [] ctxt) ctxt) THEN'
  (Rmsg' "intro mask lookup red 10" (exp_rel_tac exp_rel_info ctxt) ctxt) THEN' *)
  ...
  (Rmsg' "prove ploc reduce ArgsTyRel" (assm_full_simp_solved_with_thms_tac [#ty_repr_def_thm info, #predicate_args_thm (lookup_predicate_data info pred_name)] ctxt) ctxt) THEN'
...
fun prove_ploc_rel ctxt (info: basic_stmt_rel_info) pred_name exp_rel_info =
  ...
  (Rmsg' "prove ploc rel PredDecl" (simp_tac_with_thms [#vpr_program_ctxt_eq_thm info, #predicate_lookup_thm (lookup_predicate_data info pred_name)] ctxt) ctxt) THEN'
  (Rmsg' "prove ploc rel VprArgsTy" (simp_tac_with_thms @{thms predicate_decl.defs} ctxt) ctxt) THEN'
  (Rmsg' "prove ploc rel ArgsTyRel" (assm_full_simp_solved_with_thms_tac [#ty_repr_def_thm info, #predicate_args_thm (lookup_predicate_data info pred_name)] ctxt) ctxt) THEN'
...
fun prove_ploc_sm_rel' ctxt (info: basic_stmt_rel_info) pred_name exp_rel_info =
  (Rmsg' "prove ploc rel rule" (resolve_tac ctxt @{thms exp_rel_predicate_loc_sm'}) ctxt) THEN'
  (Rmsg' "prove ploc rel CtxtFunWf" (resolve_tac ctxt [#ctxt_wf_thm info]) ctxt) THEN'
  (Rmsg' "prove ploc rel StateRel" (fastforce_tac ctxt []) ctxt) THEN'
  (Rmsg' "prove ploc rel FunName" (simp_tac_with_thms [] ctxt) ctxt) THEN'
  (Rmsg' "prove ploc rel PredDecl" (simp_tac_with_thms [#vpr_program_ctxt_eq_thm info, #predicate_lookup_thm (lookup_predicate_data info pred_name)] ctxt) ctxt) THEN'
  (Rmsg' "prove ploc rel VprArgsTy" (simp_tac_with_thms @{thms predicate_decl.defs} ctxt) ctxt) THEN'
  (Rmsg' "prove ploc rel ArgsTyRel" (assm_full_simp_solved_with_thms_tac [#ty_repr_def_thm info, #predicate_args_thm (lookup_predicate_data info pred_name)] ctxt) ctxt) THEN'
  (Rmsg' "prove ploc rel PlocBpl" (assm_full_simp_solved_with_thms_tac [] ctxt) ctxt) THEN'
  (Rmsg' "prove ploc rel ArgsRel" (exps_rel_tac exp_rel_info ctxt) ctxt) THEN'
...
fun extract_ploc t =
  case Logic.strip_assums_concl t of
    @{term "Trueprop"} $ (_ $ (Const (@{const_name BinOp},_) $ _ $ Const (@{const_name Le},_) $ (Const (@{const_name FunExp},_) $ _ $ _ $ (_ $ (_ $ (_ $ t' $ _))))) $ _ $ _) => t'
  | _ => raise TERM ("extract ploc: match error", [t])

fun intro_fact_pred_mask_lookup_reduction ctxt (info: basic_stmt_rel_info) pred_name exp_rel_info exp_rel_perm_access_thm =
  (* SUBGOAL (fn (t,_) => (Pretty.writeln (Syntax.pretty_term ctxt (extract_ploc t)); all_tac)) THEN' *)
  (SUBGOAL (fn (t,i) =>
    (Rmsg' "intro pred mask lookup revcut" (revcut_tac (Drule.infer_instantiate ctxt [(("e_ploc_bpl", 0), Thm.cterm_of ctxt (extract_ploc t))] exp_rel_perm_access_thm)) ctxt i))) THEN'
  ...
  (Rmsg' "intro pred mask lookup PlocRel" (prove_ploc_reduce ctxt info pred_name exp_rel_info) ctxt) THEN'
