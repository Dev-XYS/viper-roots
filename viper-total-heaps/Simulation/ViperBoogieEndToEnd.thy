...
fun initial_global_state_aux :: "program \<Rightarrow> tr_vpr_bpl \<Rightarrow> 'a full_total_state \<Rightarrow> ('a vbpl_absval) named_state"
  where "initial_global_state_aux Pr Tr \<omega> x =
           (if x = heap_var Tr then
              Some (AbsV (AHeap (construct_bpl_heap_from_vpr_state Pr (field_translation Tr) (get_total_full \<omega>))))
           else
...
lemma initial_global_state_aux_heap:
  "initial_global_state_aux Pr Tr \<omega> (heap_var Tr) = Some (AbsV (AHeap (construct_bpl_heap_from_vpr_state Pr (field_translation Tr) (get_total_full \<omega>))))"
  by simp
  ...
  proof (cases rule: Set.UnE[OF *])
    ...
    proof (cases rule: Set.UnE[OF 1])
      ...
      case 1 \<comment>\<open>heap_var case\<close>
        ...
        hence "v = (AbsV (AHeap (construct_bpl_heap_from_vpr_state Pr (field_translation Tr) (get_total_full \<omega>))))" (is "v = ?h")
          using \<open>?ns x = Some v\<close>
          by simp
        have "type_of_vbpl_val T ?h = (TConSingle (THeapId T))"
          apply (simp only: vbpl_absval_ty.simps type_of_val.simps)
          using construct_bpl_heap_from_vpr_state_correct_aux[OF WfTyRepr VprHeapTy[unfolded get_hh_total_full.simps] _ InjTrField, OF refl, unfolded Let_def, THEN conjunct2, THEN conjunct2, THEN conjunct2]
          by auto
        then show ?thesis
...
proof -
  ...
  next
    ...
    next
      show "vbpl_absval_ty_opt T (AHeap (construct_bpl_heap_from_vpr_state (program_total ctxt_vpr) (field_translation Tr) (get_total_full \<omega>))) = Some (THeapId T, [])"
        using construct_bpl_heap_from_vpr_state_correct_aux[OF WfTyRepr ViperHeapWellTy[unfolded get_hh_total_full.simps] DomainTy InjFieldTr]
        by meson
    next
      show "heap_rel (program_total ctxt_vpr) (field_translation Tr) (get_hh_total_full \<omega>)
       (construct_bpl_heap_from_vpr_state (program_total ctxt_vpr) (field_translation Tr) (get_total_full \<omega>))"
        using construct_bpl_heap_from_vpr_state_correct_aux[OF WfTyRepr ViperHeapWellTy[unfolded get_hh_total_full.simps] DomainTy InjFieldTr]
        by (metis get_hh_total_full.simps)
    next
