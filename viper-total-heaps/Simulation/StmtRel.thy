theory StmtRel
  imports ExpRel ExprWfRel InhaleRel ExhaleRel TotalViperSemantics.TotalSemProperties ViperBoogieTranslationInterface Simulation TotalViperSemantics.TotalStateInst
begin
...
\<comment>\<open>Normal case\<close>
  ...
  proof -
    from RedVpr obtain v where RedEVpr: "ctxt_vpr, (Some \<omega>) \<turnstile> \<langle>e_vpr; \<omega>\<rangle> [\<Down>]\<^sub>t (Val v)" and
...
definition field_assign_normal_premise
  where "field_assign_normal_premise ctxt StateCons e_r f e \<omega> addr v ty \<equiv> 
    ctxt, (Some \<omega>) \<turnstile> \<langle>e_r; \<omega>\<rangle> [\<Down>]\<^sub>t Val (VRef (Address addr)) \<and>
    (addr,f) \<in> get_writeable_locs \<omega> \<and>
    ctxt, (Some \<omega>)  \<turnstile> \<langle>e; \<omega>\<rangle> [\<Down>]\<^sub>t Val v \<and>
...
lemma field_assign_rel_general:
  assumes SubExpsWfRel: "exprs_wf_rel (rel_ext_eq R) ctxt_vpr StateCons P ctxt [rcv_vpr,rhs_vpr] \<gamma> \<gamma>1"
      and WriteableLocRel: "wf_rel_fieldacc get_writeable_locs (rel_ext_eq R) (rel_ext_eq R) ctxt_vpr StateCons P ctxt rcv_vpr f_vpr 
                 \<gamma>1 \<gamma>2"
      and UpdHeapRel: "rel_general R R (\<lambda>\<omega> \<omega>'. \<exists>\<tau>_vpr addr v. \<omega>' = upd_hh_loc_total_full \<omega> (addr,f_vpr) v \<and>
...
proof (rule stmt_rel_intro)
  ...
  thus "\<exists>ns'. red_ast_bpl P ctxt (\<gamma>, Normal ns) (\<gamma>', Normal ns') \<and> R \<omega>' ns'"
    proof cases
      ...
      hence "red_pure_exps_total ctxt_vpr (Some \<omega>) [rcv_vpr, rhs_vpr] \<omega> (Some [VRef (Address addr), v])"
...
next
  ...
  proof cases
    ...
    hence "red_pure_exps_total ctxt_vpr (Some \<omega>) [rcv_vpr, rhs_vpr] \<omega> (Some [VRef r, v])"
      by (auto intro!: TotalExpressions.RedExpListCons TotalExpressions.RedExpListNil)
      from this obtain ns1 where
        "?Rext \<omega> \<omega> ns1" and "red_ast_bpl P ctxt (\<gamma>, Normal ns) (\<gamma>1, Normal ns1)"
        using exprs_wf_rel_normal_elim[OF SubExpsWfRel \<open>?Rext \<omega> \<omega> ns\<close>]
  ...
  next
    case RedSubExpressionFailure
    hence RedSubExpFailureAux: "red_pure_exps_total ctxt_vpr (Some \<omega>) [rcv_vpr, rhs_vpr] \<omega> None"
      by simp
    thus ?thesis
...
lemma field_assign_rel:
  assumes WfConsistency: "wf_total_consistency ctxt_vpr StateCons StateCons_t"
      ...
      \<comment>\<open>needed to preserve external consistency, see @{thm [source] total_consistency_red_stmt_extcons_preserve}\<close>
      and HeapWt: "\<And> \<omega> ns. R \<omega> ns \<Longrightarrow> total_heap_well_typed (program_total ctxt_vpr) (absval_interp_total ctxt_vpr) (get_hh_total_full \<omega>)"
  ...
  proof cases
    ...
    moreover from RedStmt have
      ...
      using total_consistency_red_stmt_preserve[OF WfConsistency] Consistent[OF _ \<open>R \<omega> ns\<close>]
        CtxtPredWf CtxtPredSF HeapWt[OF \<open>R \<omega> ns\<close>]
...
next
  ...
  next
    ...
    hence RedSubExpFailureAux: "red_pure_exps_total ctxt_vpr (Some \<omega>) [rcv_vpr, rhs_vpr] \<omega> None"
      by simp
    show ?thesis
    proof (cases  "ctxt_vpr, (Some \<omega>) \<turnstile> \<langle>rcv_vpr; \<omega>\<rangle> [\<Down>]\<^sub>t VFailure")
      case True
      then show ?thesis
    ...
    next
      case False
      from this obtain v where "ctxt_vpr, (Some \<omega>) \<turnstile> \<langle>rcv_vpr; \<omega>\<rangle> [\<Down>]\<^sub>t Val v" and
                               "ctxt_vpr, (Some \<omega>) \<turnstile> \<langle>rhs_vpr; \<omega>\<rangle> [\<Down>]\<^sub>t VFailure"
...
proof (rule field_assign_rel[OF WfConsistency CtxtPredWf CtxtPredSF, where ?\<tau>_vpr = "the (declared_fields (program_total ctxt_vpr) f_vpr)"])
  ...
  show "\<And>\<omega> ns. R \<omega> ns \<Longrightarrow> total_heap_well_typed (program_total ctxt_vpr) (absval_interp_total ctxt_vpr) (get_hh_total_full \<omega>)"
    using RStateRel state_rel_heap_well_typed \<open>domain_type TyRep = absval_interp_total ctxt_vpr\<close>
    by metis
...
proof (rule stmt_rel_intro)
  ...
  have extcons: "consistent_external ctxt_vpr (get_total_full \<omega>')"
    using Consistent WfConsistency CtxtPredWf CtxtPredSF \<open>R \<omega> ns\<close> HeapWt[OF \<open>R \<omega> ns\<close>] local.RedExhale
          total_consistency_red_stmt_extcons_preserve
    by blast
...
(* Todo: move *)
lemmas RedExhaleNormal_case' = RedExhaleNormal_case[folded nm_loc_sum.simps get_nm_total_full.simps]
...
proof (rule stmt_rel_intro)
  ...
  proof (rule RedExhaleNormal_case')
    ...
    with HavocState have "\<omega>' \<in> havoc_locs_state ctxt_vpr \<omega>_exh {}"
      using nm_loc_sum_unique
      by (smt (verit, best) dual_order.irrefl empty_Collect_eq)

    thus "\<omega> = \<omega>'"
      by (simp add: \<open>\<omega> = \<omega>_exh\<close> havoc_locs_state_empty)
...
lemma exhale_stmt_rel_inst_framing_inv:
  ...
  apply (rule exhale_stmt_rel_inst[OF WfConsistency CtxtPredWf CtxtPredSF _ _ _ ExhRel UpdHavoc])
  using assms
  by auto
...
lemma red_ast_bpl_rel_transitive_with_inv_capture_state:
  assumes Rel1: "red_ast_bpl_rel R0 R1 P ctxt \<gamma>0 \<gamma>1"
      ...
      and Rel2: "red_ast_bpl_rel (\<lambda>\<omega> ns. R1 \<omega> ns \<and> Q \<omega>) (uncurry (\<lambda>\<omega>def. state_rel_capture_total_state_kf Pr StateCons TyRep Tr FieldTr0 Kf AuxPred ctxt m h \<omega>def \<omega>def)) P ctxt \<gamma>1 \<gamma>2"
    shows "red_ast_bpl_rel R0 (uncurry (\<lambda>\<omega>def. state_rel_capture_total_state_kf Pr StateCons TyRep Tr FieldTr0 Kf AuxPred ctxt m h \<omega>def \<omega>def)) P ctxt \<gamma>0 \<gamma>2"
  using assms
...
lemma assert_stmt_rel_alt:
  assumes InvHolds: "\<And> \<omega> ns. R \<omega> ns \<Longrightarrow> Q A \<omega> \<omega>"
      ...
      and ResetState: "\<And> \<omega>0. rel_general (uncurry (RStore \<omega>0)) (\<lambda>\<omega> ns. R' (snd \<omega>) ns) 
                                          (\<lambda> \<omega>1 \<omega>2. snd \<omega>2 = \<omega>0 \<and> red_exhale ctxt_vpr \<omega>0 A \<omega>0 (RNormal (snd \<omega>1)))
                                          (\<lambda>_. False) P ctxt \<gamma>2 \<gamma>'
                             \<or> red_ast_bpl_rel R R' P ctxt \<gamma> \<gamma>'"
    shows "stmt_rel R R' ctxt_vpr StateCons \<Lambda>_vpr P ctxt (ViperLang.Assert A) \<gamma> \<gamma>'"
proof (rule stmt_rel_intro_2)
  ...
  assumes MdeclSome: "program.methods (program_total ctxt_vpr) m = Some mdecl"
      ...
      and RelPremises:
          "\<And> (\<omega>0 :: 'a full_total_state) ns0 v_args v_rets.
            ...
            red_pure_exps_total ctxt_vpr (Some \<omega>0) es \<omega>0 (Some v_args) \<Longrightarrow>
...
proof (rule stmt_rel_intro_2)
  ...
  proof (cases)
    ...
    have ListAllArgsEvalVpr: "list_all2 (\<lambda>e v. ctxt_vpr, Some \<omega>0 \<turnstile> \<langle>e; \<omega>0\<rangle> [\<Down>]\<^sub>t Val v) es v_args"
      ...
      next
        case (RNormal \<omega>post)
          ...
          thus ?thesis
            ...
            by (metis exh_if_total.elims exh_if_total_normal_2 map_result_total.simps(1) red_ast_bpl_transitive result_total.distinct(5))
      qed
    qed
  next
    case RedSubExpressionFailure
...
\<comment>\<open>Arguments are well-defined by default since we restrict them to be variables here\<close>
  ...
  proof (rule wf_rel_intro)
    ...
    assume "rel_ext_eq R \<omega>def \<omega> ns" and 
           "red_pure_exps_total ctxt_vpr (Some \<omega>def) es \<omega> None"

    from this obtain i where "i < length es" and 
                             "red_pure_exp_total ctxt_vpr (Some \<omega>def) (es ! i) \<omega> VFailure"
...
\<comment>\<open>Initialization\<close>
  ...
  assume R0: "R \<omega> ns"
     and RedArgs: "red_pure_exps_total ctxt_vpr (Some \<omega>) es \<omega> (Some v_args)"
  ...
  have ListAllArgsEvalVpr: "list_all2 (\<lambda>e v. ctxt_vpr, Some \<omega> \<turnstile> \<langle>e; \<omega>\<rangle> [\<Down>]\<^sub>t Val v) es v_args"
  ...
  have StoreValArgsVpr: "list_all2 (\<lambda>x v. get_store_total \<omega> x = Some v) xs v_args"
    proof -
      have "list_all2 (\<lambda>x v. ctxt_vpr, Some \<omega> \<turnstile> \<langle>ViperLang.Var x; \<omega>\<rangle> [\<Down>]\<^sub>t Val v) xs v_args"
      ...
      next
        ...
        hence *: "ctxt_vpr, Some \<omega> \<turnstile> \<langle>es ! i; \<omega>\<rangle> [\<Down>]\<^sub>t Val (v_args ! i)"
        ...
        thus "ctxt_vpr, Some \<omega> \<turnstile> \<langle>pure_exp.Var (xs ! i);\<omega>\<rangle> [\<Down>]\<^sub>t Val (v_args ! i)"
          using *
          by simp
      qed
      thus ?thesis
  ...
  proof -
    ...
    proof (rule state_rel_store_update[where ?f= var_tr'])
      show "consistent_state_rel_opt (state_rel_opt (Tr\<lparr>var_translation := var_tr'\<rparr>)) \<Longrightarrow> StateCons ?\<omega>0 \<and> consistent_external (total_context.make Pr (\<lambda>_. None) (domain_type TyRep)) (get_total_full ?\<omega>0)"
        apply (intro conjI)
        ...
        by (metis ConsistencyEnabled StateRel full_total_state.select_convs(3) state_rel_consistent)
    next
      show "binder_state ns = Map.empty"
    ...
    proof -
      ...
      proof (rule allI, rule impI)+
        ...
        ultimately show "res \<noteq> RFailure"
          ...
          by (smt (verit, ccfv_SIG) WfConsistency \<open>StateCons_t (get_total_full \<omega>)\<close> full_total_state.select_convs(1) full_total_state.select_convs(2) map_upd_Some_unfold option.discI wf_total_consistency_def)
      qed
     ...
     \<comment>\<open>using that return variables do not appear in precondition\<close>
      ...
      show ?thesis
      proof (rule framing_exhI[OF _ _ _ AssertionFraming_\<omega>0'_only_args])
...
\<comment>\<open>We next show that the havocs capture the nondeterministic assignments to the return variables\<close>
  ...
  proof (rule rel_intro)
    ...
    proof -
     ...
     next
       ...
       show "StateCons \<omega>havoc \<and> consistent_external (total_context.make Pr (\<lambda>_. None) (domain_type TyRep)) (get_total_full \<omega>havoc)"
         unfolding \<open>\<omega>havoc = _\<close>
         apply (intro conjI)
          apply (rule total_consistencyI[OF WfConsistency])
     ...
     proof -
        ...
        proof -
          ...
          proof -
            have "get_hh_total_full (?\<omega>0_rets \<ominus> ?\<omega>pre_exh_aux_rets) = get_hh_total_full ?\<omega>0_rets"
              using minus_full_total_state_only_mask_different
              by blast
            hence *: "get_hh_total_full (?\<omega>0_rets \<ominus> ?\<omega>pre_exh_aux_rets) = get_hh_total_full ?\<omega>0_rets_empty"
              by simp
          ...
          proof (rule allI | rule impI)+
            ...
            show "res \<noteq> RFailure"
              using MethodSpecsFramed
              unfolding vpr_method_spec_correct_total_def vpr_method_correct_total_aux_def
              by (smt (verit, best) WfConsistency \<open>StateCons_t (get_total_full \<omega>)\<close> full_total_state.select_convs(2) full_total_state.simps(5) full_total_state.simps(7) map_upd_Some_unfold option.discI wf_total_consistency_def)
          qed
        ...
        ultimately have PostFramedAuxSmaller: "vpr_postcondition_framed ctxt_vpr StateCons (method_decl.post mdecl) (get_total_full (?\<omega>0_rets \<ominus> ?\<omega>pre_exh_aux_rets)) (get_store_total ?\<omega>0_rets)"
          using MethodSpecsFramed
          unfolding vpr_method_spec_correct_total_def vpr_method_correct_total_aux_def
          by (smt (verit, ccfv_SIG) WfConsistency \<open>StateCons_t (get_total_full \<omega>)\<close> full_total_state.select_convs(2) full_total_state.update_convs(1) inhale_only_changes_mask map_upd_Some_unfold minus_full_total_state_only_mask_different option.discI wf_total_consistency_def)
      ...
      proof (rule allI, rule impI)+
        ...
        let ?\<omega>havoc2 = "\<lparr> get_store_total = get_store_total ?\<omega>0_rets,
          ...
          ultimately show "res \<noteq> RFailure"
            ...
            by (smt (verit, best) SuccessHavoc WfConsistency \<open>StateCons_t (get_total_full \<omega>)\<close> full_total_state.select_convs(2) full_total_state.select_convs(3) map_upd_Some_unfold option.discI wf_total_consistency_def)
        qed
      qed
  ...
  proof (rule rel_general_success_refl_2)
    ...
    using SuccessReset inhale_only_changes_mask
    ...
    proof -
      ...
      proof (rule state_rel_store_update[where ?f="var_translation Tr"])
            ...
            proof -
              ...
              next
                ...
                have *: "map_upd_set AuxPred (ran (var_translation Tr) - (set xs_bpl \<union> set ys_bpl))
                  ...
                  using \<open>var_bpl \<notin> set xs_bpl\<close> \<open>var_bpl \<notin> set ys_bpl\<close> VarTrSome ranI
                  by fast
        ...
        qed
      next
        have cons2: "StateCons_t (get_total_full \<omega>post) \<and>
                     consistent_external (total_context.make Pr (\<lambda>_. None) (domain_type TyRep)) (get_total_full \<omega>post)"
        proof -
          have "consistent_state_rel_opt (state_rel_opt (Tr\<lparr>var_translation := var_tr''\<rparr>))"
            by (simp add: ConsistencyEnabled)
        ...
        show "StateCons (reset_state_after_call ys v_rets \<omega> \<omega>post) \<and>
              consistent_external (total_context.make Pr (\<lambda>_. None) (domain_type TyRep)) (get_total_full (reset_state_after_call ys v_rets \<omega> \<omega>post))"
          unfolding reset_state_after_call_def
          apply (intro conjI)
           apply (rule total_consistencyI[OF WfConsistency])
            apply (simp add: cons2)
...
proof (rule stmt_rel_intro_2)
  ...
  from RedStmtVpr obtain res_body v where
    ...
    and ResEqUnshift: "res = map_result_total (unshift_state_total (Suc 0)) res_body"
  ...
  proof (rule rel_vpr_aux_intro)
    ...
    with ResEqUnshift obtain \<omega>_body where
      ...
      by (blast elim: map_result_total.elims)
    ...
    proof (rule state_rel_store_update[OF StateRelBodyEnd, where ?f = ?var_tr])
      ...
      proof -
        ...
        from state_rel_consistent[OF StateRel_ns' \<open>?ConsOpt\<close>] have
          ...
          by force
        have HeapWtOmega: "total_heap_well_typed (program_total ctxt_vpr) (absval_interp_total ctxt_vpr) (get_hh_total_full \<omega>)"
          using state_rel_heap_well_typed[OF StateRel_ns'] CtxtProg DomainTyRep
          by simp
        from total_consistency_red_stmt_extcons_preserve[OF WfConsistency ExtConsOmega CtxtPredWf CtxtPredSF StateConsOmega
               HeapWtOmega RedStmtVpr[simplified \<open>res = RNormal \<omega>'\<close>]]
        show ?thesis
...
proof (rule exp_rel_equiv_vpr[OF _ assms])
  ...
  assume "ctxt_vpr, \<omega>_def_opt \<turnstile> \<langle>Binop (ELit (ViperLang.LBool True)) BImp e_vpr;\<omega>\<rangle> [\<Down>]\<^sub>t Val v1"
  thus "ctxt_vpr, \<omega>_def_opt \<turnstile> \<langle>e_vpr;\<omega>\<rangle> [\<Down>]\<^sub>t Val v1"
  ...
  next
    ...
    assume "ctxt_vpr, \<omega>_def_opt \<turnstile> \<langle>e_vpr;\<omega>\<rangle> [\<Down>]\<^sub>t Val v2"
    thus "ctxt_vpr, \<omega>_def_opt \<turnstile> \<langle>e_vpr;\<omega>\<rangle> [\<Down>]\<^sub>t Val v1"
