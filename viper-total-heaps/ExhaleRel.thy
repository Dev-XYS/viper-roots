...
next
  show "valid_heap_mask (get_mh_total_full \<omega>)"
    ...
    by blast
next
  show "upd_nm_total_full \<omega> 0 \<oplus> \<omega> = Some \<omega>"
...
lemma exhale_rel_imp_3:
  assumes Invariant: "\<And>\<omega>def \<omega>. ctxt_vpr, Some \<omega>def \<turnstile> \<langle>cond; \<omega>\<rangle> [\<Down>]\<^sub>t (Val (VBool True)) \<Longrightarrow> Q (assert.Imp cond A) \<omega>def \<omega> \<Longrightarrow> Q A \<omega>def \<omega>"
      and ExpWfRel: 
          "expr_wf_rel (\<lambda> \<omega>def \<omega> ns. R \<omega>def \<omega> ns \<and> Q (assert.Imp cond A) \<omega>def \<omega>) ctxt_vpr StateCons P ctxt cond 
           \<gamma>1 \<gamma>_if" 
      and ThnRel: "exhale_rel (\<lambda>\<omega>def \<omega> ns. R \<omega>def \<omega> ns \<and> ctxt_vpr, Some \<omega>def \<turnstile> \<langle>cond; \<omega>\<rangle> [\<Down>]\<^sub>t (Val (VBool True)))
                              R Q ctxt_vpr StateCons P ctxt A \<gamma>_if \<gamma>'"
      and ElsRel: "rel_general (uncurry R) (uncurry R) (\<lambda>\<omega> \<omega>'. \<omega> = \<omega>' \<and> ctxt_vpr, Some (fst \<omega>)  \<turnstile> \<langle>cond; (snd \<omega>)\<rangle> [\<Down>]\<^sub>t (Val (VBool False))) (\<lambda>_. False) P ctxt \<gamma>_if \<gamma>'"
...
proof (simp only: uncurry.simps, 
  ...
  let ?R1Thn = "(\<lambda>\<omega> ns. R (fst \<omega>) (snd \<omega>) ns \<and> ctxt_vpr, Some (fst \<omega>)  \<turnstile> \<langle>cond; snd \<omega>\<rangle> [\<Down>]\<^sub>t (Val (VBool True)) \<and> Q A (fst \<omega>) (snd \<omega>))"
  let ?SuccessThn = "\<lambda>\<omega> \<omega>'. fst \<omega> = fst \<omega>' \<and> red_exhale ctxt_vpr (fst \<omega>) A (snd \<omega>) (RNormal (snd \<omega>'))"  
  let ?FailThn = "\<lambda>\<omega>. red_exhale ctxt_vpr (fst \<omega>) A (snd \<omega>) RFailure"
  ...
  show "\<And>\<omega> \<omega>' ns.
       fst \<omega> = fst \<omega>' \<and> red_exhale ctxt_vpr (fst \<omega>) (assert.Imp cond A) (snd \<omega>) (RNormal (snd \<omega>')) \<Longrightarrow>
       ...
       ((R (fst \<omega>) (snd \<omega>) ns \<and> ctxt_vpr, Some (fst \<omega>) \<turnstile> \<langle>cond;snd \<omega>\<rangle> [\<Down>]\<^sub>t Val (VBool True) \<and> Q A (fst \<omega>) (snd \<omega>)) \<and>
        fst \<omega> = fst \<omega>' \<and> red_exhale ctxt_vpr (fst \<omega>) A (snd \<omega>) (RNormal (snd \<omega>')) \<or>
        uncurry R \<omega> ns \<and> \<omega> = \<omega>' \<and> ctxt_vpr, Some (fst \<omega>) \<turnstile> \<langle>cond;snd \<omega>\<rangle> [\<Down>]\<^sub>t Val (VBool False))"
  ...
  show "\<And>\<omega> ns.
       red_exhale ctxt_vpr (fst \<omega>) (assert.Imp cond A) (snd \<omega>) RFailure \<Longrightarrow>
       R (fst \<omega>) (snd \<omega>) ns \<and> Q (assert.Imp cond A) (fst \<omega>) (snd \<omega>) \<Longrightarrow>
       ctxt_vpr, Some (fst \<omega>) \<turnstile> \<langle>cond;snd \<omega>\<rangle> [\<Down>]\<^sub>t VFailure \<or>
       (\<omega> = \<omega> \<and> (\<exists>v. ctxt_vpr, Some (fst \<omega>) \<turnstile> \<langle>cond;snd \<omega>\<rangle> [\<Down>]\<^sub>t Val v)) \<and>
       ((R (fst \<omega>) (snd \<omega>) ns \<and> ctxt_vpr, Some (fst \<omega>) \<turnstile> \<langle>cond;snd \<omega>\<rangle> [\<Down>]\<^sub>t Val (VBool True) \<and> Q A (fst \<omega>) (snd \<omega>)) \<and>
        red_exhale ctxt_vpr (fst \<omega>) A (snd \<omega>) RFailure \<or>
...
lemma exhale_rel_pure_general:
  assumes Wf: "expr_wf_rel (\<lambda> \<omega>def \<omega> ns. R \<omega>def \<omega> ns \<and> Q (Atomic (Pure e_vpr)) \<omega>def \<omega>) ctxt_vpr StateCons P ctxt e_vpr \<gamma> \<gamma>1"
      and ExpCheckRel: "rel_general (\<lambda> \<omega>def_\<omega> ns. R (fst \<omega>def_\<omega>) (snd \<omega>def_\<omega>) ns \<and> Q (Atomic (Pure e_vpr)) (fst \<omega>def_\<omega>) (snd \<omega>def_\<omega>))
                           (uncurry R')
                           (\<lambda> \<omega>def_\<omega> \<omega>def_\<omega>'. \<omega>def_\<omega> = \<omega>def_\<omega>' \<and> ctxt_vpr, Some (fst \<omega>def_\<omega>) \<turnstile> \<langle>e_vpr; (snd \<omega>def_\<omega>)\<rangle> [\<Down>]\<^sub>t Val (VBool True))
                           (\<lambda> \<omega>def_\<omega>. ctxt_vpr, Some (fst \<omega>def_\<omega>) \<turnstile> \<langle>e_vpr; (snd \<omega>def_\<omega>)\<rangle> [\<Down>]\<^sub>t Val (VBool False))
...
proof (rule exhale_rel_pure_general, rule Wf, rule rel_intro)
  ...
  assume R: "R (fst \<omega>) (snd \<omega>) ns \<and> Q (Atomic (Pure e_vpr)) (fst \<omega>) (snd \<omega>)"
     and Succ: "\<omega> = \<omega>' \<and> ctxt_vpr, Some (fst \<omega>) \<turnstile> \<langle>e_vpr;snd \<omega>\<rangle> [\<Down>]\<^sub>t Val (VBool True)"

  with ExpRel have RedExpBpl: "red_expr_bpl ctxt e_bpl ns (BoolV True)"
    using R
    by (metis eval_with_None exp_rel_vpr_bpl_elim val_rel_vpr_bpl.simps(2))
...
next
  fix \<omega> ns
  assume R: "R (fst \<omega>) (snd \<omega>) ns \<and> Q (Atomic (Pure e_vpr)) (fst \<omega>) (snd \<omega>)"
     and Fail: "ctxt_vpr, Some (fst \<omega>) \<turnstile> \<langle>e_vpr;snd \<omega>\<rangle> [\<Down>]\<^sub>t Val (VBool False)"
