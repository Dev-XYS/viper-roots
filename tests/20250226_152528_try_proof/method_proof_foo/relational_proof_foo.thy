...
CtxtVprWfPred: "ctxt_wf_pred ctxt_vpr" and
CtxtVprPredSelfFraming: "ctxt_pred_self_framing ctxt_vpr"
begin
...
val basic_stmt_rel_info = {ctxt_wf_thm = @{thm CtxtWf}, consistency_wf_thm = @{thm wf_total_consistency_internal[OF CtxtVprWfPred CtxtVprPredSelfFraming]}, consistency_down_mono_thm = @{thm true_mono_prop_downward_ord}, tr_def_thm = @{thm tr_vpr_bpl_0_def}, method_data_table = method_decl_data, vpr_program_ctxt_eq_thm = @{thm VprProgramTotal}, var_rel_tac = lookup_var_rel_tac, var_context_vpr_tac = assm_full_simp_solved_with_thms_tac @{thms var_ctxt_viper_def shift_and_add_def}, field_rel_single_tac = field_rel_single_tac, aux_var_disj_tac = aux_var_disj_tac, type_interp_econtext = @{thm TyInterpBpl}, vpr_prog_def_thm = @{thm vpr_prog_def}}
...
val stmt_body_hints = (AtomicHint (InhaleHint {inhale_stmt_rel_thm = @{thm inhale_stmt_rel_no_inv}, inhale_rel_hint = (GoodStateAfter (GoodStateAfter (AtomicInhHint (PredicateAccInhHint (exp_wf_rel_info, exp_rel_info, @{thm foo_before_ast_to_cfg_prog.lvar8(2)}, @{thm state_rel_aux_pred_sat_lookup_3[where ?aux_var="8"]})))))}))
\<close>
